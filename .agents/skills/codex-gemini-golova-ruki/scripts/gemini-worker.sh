...
usage() {
  cat >&2 <<'EOF'
Usage:
  gemini-worker.sh <scout|executor|verifier|reviewer> <prompt-file> [report-file]

Examples:
  gemini-worker.sh scout .agent-runs/run/scout-prompt.md .agent-runs/run/scout-report.md
  gemini-worker.sh executor .agent-runs/run/executor-prompt.md .agent-runs/run/executor-report.md
EOF
}
...
case "$ROLE" in
  scout)
    ROLE_RULES=$(cat <<'EOF'
You are a fresh Gemini repository scout working for a Codex head.
Operate read-only even though tool permissions are auto-approved.
Do not edit, create, delete, rename, commit, push, install dependencies, mutate GitHub, deploy, send transactions, or make product decisions.
...
EOF
  ...
  executor)
    ROLE_RULES=$(cat <<'EOF'
You are a fresh Gemini coding executor working for a Codex head.
...
EOF
  ...
  verifier)
    ROLE_RULES=$(cat <<'EOF'
You are a fresh Gemini DoD verifier working for a Codex head.
Operate read-only even though tool permissions are auto-approved.
Read the issue-spec and current diff, run allowed checks, and verify every DoD item independently.
Do not edit code, repair failures, commit, push, install dependencies, mutate GitHub, deploy, send transactions, or make product decisions.
...
EOF
  ...
  reviewer)
    ROLE_RULES=$(cat <<'EOF'
You are a fresh Gemini adversarial reviewer working for a Codex head.
Operate read-only even though tool permissions are auto-approved.
...
Do not edit code, repair findings, commit, push, install dependencies, mutate GitHub, deploy, send transactions, or make product decisions.
...
Role: $ROLE
Workflow: codex-gemini-golova-ruki
Model contract: Gemini 3.1 Pro (High) hands working for the Codex head.
...
- The Codex head owns unresolved decisions and final acceptance.
...

{
  ...
  printf '_Workflow: codex-gemini-golova-ruki_\n\n'
  printf '_Model: %s_\n\n' "$MODEL"
