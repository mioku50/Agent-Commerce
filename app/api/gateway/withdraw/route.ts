...
function getSupabase() {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error(
      "Supabase environment variables are required to record withdrawals.",
    );
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error: `Seller wallet (${gateway.address}) has no native USDC on Arc Testnet to pay for gas fees. Fund it with testnet USDC first.`,
        },
        { status: 400 },
      );
    }
  ...
  try {
    ...
    if (
      ...
      message = isCrossChain
        ...
        : `Seller wallet has insufficient native USDC to pay for gas on Arc Testnet. Fund ${gateway.address} with testnet USDC and retry.`;
    }
