...
export async function runDeterministicEvaluationPolicy(input: {
  ...
  // 3. Job Status = Submitted (1)
  const isSubmitted = input.onchainJob.status === 1;
  checks.push({
    ...
    severity: "critical",
    message: isSubmitted
      ? "ERC-8183 Job status is Submitted."
      : `Job status is ${input.onchainJob.status}, expected Submitted (1).`,
  });
  if (!isSubmitted) {
