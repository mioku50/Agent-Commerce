...
const TARGET_VERSION = "20260801120000";
const TARGET_FILE = `${TARGET_VERSION}_p42_project_360.sql`;
...
async function main() {
  ...
  const migrations = readdirSync(migrationsDirectory)
    ...
    .sort();
  assert(migrations.at(-1) === TARGET_FILE, "Project 360 is not the final ordered local migration.");
  const versions = migrations.map((name) => name.split("_", 1)[0]);
  assert(
    versions.filter((version) => version === TARGET_VERSION).length === 1,
    "The Project 360 target migration version is not unique.",
  );
  ...
  const sql = readFileSync(path.join(migrationsDirectory, TARGET_FILE), "utf8");
  ...
  try {
    ...
    if (applied.has(TARGET_VERSION)) {
      assert(presentCount === 4, "Migration ledger is applied but Project 360 schema is incomplete.");
      console.log("[project-360-readiness] PASS: ordered migration is already applied and schema is present.");
    } else {
